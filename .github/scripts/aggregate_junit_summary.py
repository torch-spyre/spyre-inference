...
totals plus a single searchable row for every test case across all shards.

The table carries a per-case Shard column (each case's junit-<target>.xml file
name) for provenance, but no per-shard rollup rows -- the point is one flat
ctrl+f-able index of every case, not a per-shard breakdown.
"""
