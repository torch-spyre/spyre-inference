...
Supports two XML types:
  ...
  2. Performance benchmark XMLs (classname contains ".benchmark")
     --> benchmark_runs / perf_benchmarks
...
def parse_benchmark_xml(xml_path: Path):
    ...
    for tc in suite.findall(".//testcase"):
        ...
        if not m:
            print(f"  [warn] Unrecognised benchmark name pattern: {name}", file=sys.stderr)
            continue
        op = m.group("op")
...
def parse_test_xml(xml_path: Path):
    ...
    for tc in suite.findall(".//testcase"):
        ...
        op_name, dtype, platform = extract_op_dtype_platform(tc.get("name", ""), properties)
        raw_cases.append(
            {
    ...
    for xml_path in xml_files:
        ...
        if is_benchmark_xml(root):
            ...
            if existing.result_rows[0][0] > 0:
                print(f"  Already ingested benchmark — skipping {run_meta['source_file']}")
                continue
