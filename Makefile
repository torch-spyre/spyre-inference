...
.PHONY: help test tests run-one aiu-setup perf-tests coverage print-test-type \
        test-smoke test-smoke-shard test-model-quality test-probes test-probes-shard \
...
tests-single-card: ## Run the 1-card marker combos (smoke shards / model-quality / attention shards / encoder-attention / upstream shards). Needs 1 card.
	mkdir -p "$(RESULTS_DIR)"; \
	rc=0; \
