...
The models are too large to run through transformers in CI, so the references live in
`tests/data/` and are regenerated by hand where the weights are cached:
