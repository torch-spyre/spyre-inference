...
each decoder case prints how many reference steps it matched. Read that line rather than
the pass/fail alone: a prompt that stops after a step or two gates very little, and the fix
is a more confident prompt, not a looser tolerance.
