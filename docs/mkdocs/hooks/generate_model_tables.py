...
MODEL_CONFIG_PATH = ROOT_DIR / "spyre_inference/config/model_configs.yaml"
...
def on_startup(command: Literal["build", "gh-deploy", "serve"], dirty: bool):
    ...
    # MODEL_CONFIG_PATH and SUPPORTED_MODELS_PATH do not exist yet for spyre-inference.
    # This hook will activate automatically once both files are added.
    if not MODEL_CONFIG_PATH.exists() or not SUPPORTED_MODELS_PATH.exists():
        print(f"Skipping model table generation: {MODEL_CONFIG_PATH} or {SUPPORTED_MODELS_PATH} not found")
        return
