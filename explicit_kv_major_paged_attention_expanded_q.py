...
"""Repro: paged attention with page-expanded query gathers."""
...
def paged_attention(
    ...
    page_batches = entries * NUM_KV_BLOCKS
    q_pages = query_staging.index_select(0, row_index_tensor).view(
        page_batches, NUM_QUERIES_PER_KV, HEAD_SIZE
    )
    k_pages = k_pages.index_select(0, page_index).view(page_batches, BLOCK_SIZE, HEAD_SIZE)
    v_pages = v_pages.index_select(0, page_index).view(page_batches, BLOCK_SIZE, HEAD_SIZE)
    scores_pages = torch.matmul(q_pages, k_pages.transpose(-2, -1)) * scale
    scores = (
        scores_pages.view(
            NUM_KV_HEADS,
            NUM_Q_TOKENS,
        ...
        .permute(0, 1, 3, 2, 4)
        .reshape(entries, NUM_QUERIES_PER_KV, KV_LEN)
    )
    probs = torch.softmax(scores + mask_by_entry.view(entries, KV_LEN).unsqueeze(1), dim=-1)
    page_probs = (
        probs.view(
            NUM_KV_HEADS,
            NUM_Q_TOKENS,
        ...
        .permute(0, 1, 3, 2, 4)
        .reshape(page_batches, NUM_QUERIES_PER_KV, BLOCK_SIZE)
    )
    out = (
        torch.matmul(page_probs, v_pages)
        .view(
            NUM_KV_HEADS,
            NUM_Q_TOKENS,
            NUM_KV_BLOCKS,
    ...
    return out.permute(1, 0, 2, 3).reshape(NUM_Q_TOKENS, NUM_Q_HEADS, HEAD_SIZE)
...
def sdpa_reference(
    ...
    page_batches = entries * NUM_KV_BLOCKS
    q = (
        query_staging.index_select(0, row_index_tensor)
        .view(
            NUM_KV_HEADS,
            NUM_Q_TOKENS,
            NUM_KV_BLOCKS,
        ...
        .reshape(entries, NUM_QUERIES_PER_KV, HEAD_SIZE)
    )
    k = (
        k_pages.index_select(0, page_index)
        .view(page_batches, BLOCK_SIZE, HEAD_SIZE)
        .view(entries, KV_LEN, HEAD_SIZE)
    )
    v = (
        v_pages.index_select(0, page_index)
        .view(page_batches, BLOCK_SIZE, HEAD_SIZE)
        .view(entries, KV_LEN, HEAD_SIZE)
    )
...
def main():
    ...
    row_index_tensor = (
        (
            torch.arange(NUM_KV_HEADS, dtype=torch.int32)[:, None, None] * NUM_STAGING_TOKENS
            + torch.arange(NUM_Q_TOKENS, dtype=torch.int32)[None, :, None]
        )
        .expand(-1, -1, NUM_KV_BLOCKS)
        .reshape(-1)
    )
