...
class SpyreParallelLMHead(ParallelLMHead):
    ...
    def _apply(self, fn, recurse=True):
        """Override to keep LMHead weights on CPU.

        This avoids unintended device movement when model.to()/compile() is invoked,
        and preserves existing CPU execution behavior.
        """
        return self
    ...
    def forward_oot(
        ...
        **kwargs,
        ...
        # Custom op boundary (outside torch.compile)
...
def _op_func(
    ...
    if output.shape != result.shape:
        raise RuntimeError("Output shape mismatch in spyre_parallel_lm_head")
