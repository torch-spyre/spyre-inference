...
class TorchSpyrePlatform(CpuPlatform):
    ...
    # Hard caps for the on-device KV cache. Spyre's VFIO DMA region is
    # much smaller than vLLM's CPU defaults; the KV-cache budget that
    # `num_gpu_blocks_override` derives from `max_num_seqs * ceil(max_model_len
    # / block_size)` (see `check_and_update_config` below) has to fit. Anything
    # higher trips `RAS::VFIO::MapDMAFailed` during `_initialize_kv_caches`.
    # A subclass can raise these on hardware with a larger DMA region.
