...
class _PreClassifierHead(nn.Module):
    """Head that optionally runs pre_clf → ReLU before classifier, then squeezes [B,1,C]→[B,C].

    ``SequenceClassificationMixin`` wraps ``self.classifier.__class__`` with
    ``ClassifierWithReshape`` which unsqueezes the input to [B,1,hidden] before the linear,
    producing [B,1,num_labels]. ``ClassificationOutput.from_base`` requires 1-D per-request
    tensors; the squeeze step corrects that. When ``pre_clf`` is not ``None`` (e.g. DistilBERT)
    the pre_classifier → ReLU step runs first.
    ...
    def __init__(  # noqa: N805
        self_inner,
        classifier: nn.Module,
        pre_clf: nn.Module | None = None,
    ) -> None:
        super().__init__()
    ...
    def forward(self_inner, x: torch.Tensor) -> torch.Tensor:  # noqa: N805
        if self_inner.pre_clf is not None:
            x = nn.functional.relu(self_inner.pre_clf(x))
...
def _stamp_layer_idx(model: nn.Module) -> None:
    ...
    ``create_attention_instances`` uses to assign indices 0..N-1. The guard
    below enforces that invariant: if an unexpected *SelfAttention (e.g. a
    cross-attention or pooling-attention) is present, the stamped indices would
    diverge from the 0..N-1 keys in ``attention_instances`` and
    ``vllm_attention_forward`` would silently index the wrong instance.
    """
    idx = 0
    ...
    # Only validate when we actually stamped something. idx==0 means all
    # *SelfAttention modules already carried layer_idx (e.g. BERT/RoBERTa set it
    # in their constructor) — nothing was stamped, so there is nothing to validate.
    if idx > 0:
        num_hidden_layers = getattr(getattr(model, "config", None), "num_hidden_layers", None)
        if num_hidden_layers is not None and idx != num_hidden_layers:
            raise RuntimeError(
                f"_stamp_layer_idx found {idx} *SelfAttention modules but "
                f"config.num_hidden_layers={num_hidden_layers}. "
                "layer_idx stamping would produce wrong attention-instance lookups. "
                "This model likely has an extra attention module (cross-attention or "
                "pooling-attention) not accounted for — extend _stamp_layer_idx for it."
            )
...
class SpyreTransformersForSequenceClassification(TransformersForSequenceClassification):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        SpyreTransformersForCausalLM._fix_generic_config(vllm_config)
        # vLLM defaults pooling head_dtype to fp32; Spyre has no fp32 batchmatmul
        # (torch-spyre#1794). Set head_dtype="model" so vLLM resolves it to the
        # model dtype (fp16), keeping the classification head on Spyre.
    ...
    def _install_head(self) -> None:
        ...
        if pre_classifier is None:
            new_head = _PreClassifierHead(self.classifier)
        elif isinstance(pre_classifier, nn.Linear):
            new_head = _PreClassifierHead(self.classifier, pre_clf=pre_classifier)
        else:
