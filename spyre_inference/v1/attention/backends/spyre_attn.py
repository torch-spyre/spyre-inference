...
class SpyreEncoderAttentionImpl(SpyreAttentionImpl):
    ...
    ``attn_type`` branch. Setup is shared with the paged decoder impl; the
    forward runs one batched, padded SDPA via ``forward_encoder_attention``.
    """
    ...
    def forward(
        ...
        # Lazy import to avoid a circular import: spyre_encoder_attn imports
        # _overwrite from this module at the top.
        from spyre_inference.v1.attention.backends.spyre_encoder_attn import (
            forward_encoder_attention,
        )
        ...
        return forward_encoder_attention(
            query[:n],
            key[:n],
            value[:n],
            output[:n],
