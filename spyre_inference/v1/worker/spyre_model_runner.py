...
class TorchSpyreModelRunner(GPUModelRunner):
    ...
    def _determine_batch_execution_and_padding(
        ...
        """
        if self.spyre_shape_bucketer is None or not self.spyre_shape_bucketer.is_warmed_up:
        ...
        if desc is None:
            logger.warning(
                "Batch size %d exceeds max bucket %d; running without "
                "bucket padding (may trigger recompilation).",
                num_tokens,
                self.spyre_shape_bucketer.max_bucket_size,
            )
            return super()._determine_batch_execution_and_padding(
                num_tokens=num_tokens,
                num_reqs=num_reqs,
