...
from spyre_inference.custom_ops.linear import _STICK
...
def test_configure_pooling_fp32_classifier_downcasts_to_fp16():
    ...
    assert model.classifier.spyre_bias_folded is True
    assert model.classifier.weight.shape[0] == 8 + _STICK
    assert model.classifier.spyre_bias_stick is not None
    assert tuple(model.classifier.spyre_bias_stick.shape) == (1, _STICK)
...
def test_configure_pooling_roberta_head_dense_and_out_proj_downcast():
    ...
    assert head.dense.bias is None
    assert head.dense.spyre_bias_folded is True
    assert model.pooler.head.head_dtype == torch.float16


def test_spyre_classifier_linear_matches_nn_linear_on_cpu():
    """Short-row CLS (B=1) and unaligned out_proj (N=1) must match F.linear."""
    torch.manual_seed(0)
    dense = nn.Linear(8, 8)
    x = torch.randn(1, 8)
    ...
    assert wrapped_dense.weight.shape[0] == 8 + _STICK
    ...
    # bert-base-NER dummy_run: packed tokens [16, 768] -> [16, 9], not CLS [1, H].
    ner = nn.Linear(768, 9)
    x_ner = torch.randn(16, 768)
    ner_ref = ner(x_ner).detach()
    wrapped_ner = SpyreClassifierLinear.convert(ner)
    assert wrapped_ner.weight.shape[0] == 768 + _STICK
...
def test_spyre_classifier_linear_cls_warmup_shape_compiles_on_spyre():
    """B=1, H=1024 + bias is the bge-reranker dense that mixed-EA'd isolated F.linear."""
    if not spyre_available():
        pytest.skip("needs Spyre: mixed-EA bias add is a device compile failure")
    ...
    # Width-1 ones-column concat died here: stick scatter [16, 1] into [16, 769].
