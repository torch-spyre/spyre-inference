...
@pytest.mark.parametrize("num_tokens", [1, 2, 3, 32])
def test_rotary_forward_oot_on_spyre(
    ...
    """forward_oot gathers the rotation slice on the fly and runs the 2x2 rotation on
    Spyre, matching forward_native across head_size (aligned 128/256, pad-to-stick 64),
    GQA, and 2D/3D layouts. num_tokens spans single-token decode (T=1) through prefill
    (T=32) as an explicit on-device-gather regression guard (torch-spyre#3418)."""
    from vllm.model_executor.layers.rotary_embedding import get_rope
    from vllm.model_executor.layers.rotary_embedding.base import RotaryEmbedding
...
@pytest.mark.rotary
@pytest.mark.parametrize("head_size", HEAD_SIZES)
def test_rotation_isolated_across_layers(default_vllm_config, head_size):
    """Two distinct rope modules (different rope_theta -> different rotations) each
    gather their own rotation slice on the fly in forward_oot and match their own
    reference. A shared-state bug would rotate one module with the other's frequencies
    and fail the per-module assert_close."""
    from vllm.model_executor.layers.rotary_embedding import get_rope
    from vllm.model_executor.layers.rotary_embedding.base import RotaryEmbedding
