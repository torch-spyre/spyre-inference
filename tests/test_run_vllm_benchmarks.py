...
def test_serve_tests_yaml_derives_consistently():
    ...
    for config in configs:
        ...
        # A request longer than max-model-len is rejected, so the name has to say
        # which trace the entry replays.
        assert f"_{config['server_parameters']['max-model-len'] // 1024}k" in config["test_name"]
